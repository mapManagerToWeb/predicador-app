...
- **Frontend** is highly cohesive (~0.98–0.99) around `map-geometry.ts` (`pointInPolygon`, `projectOnSegment`; `snapToContour`/`traceContourBetween` were removed with the point-trace partial mode, ADR 0008) and `map-style.service.ts`, plus `profile.ProfilePage.save`.
