...
- **Production DB is a self-hosted `postgres` container** (`postgis/postgis:18-3.6`, volume `postgres_data`, port bound to `127.0.0.1:5432`) in `docker-compose.yml`, replacing Neon (ADR 0006). Only `DB_PASSWORD` is required in `.env`; `DB_URL`/`DB_USERNAME` default to the container. One-time data copy from Neon: `NEON_URL=... ./scripts/db/migrate-from-neon.sh` (dumps land in the gitignored `backups/`). The Neon-era tuning (HikariCP `minimum-idle: 0`, `management.health.db.enabled: false`, Eureka health on `/actuator/info`, Flyway via `DB_URL_UNPOOLED`; ADR 0003) is still in place but no longer required.
