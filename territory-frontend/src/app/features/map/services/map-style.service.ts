...
<<<<<<< HEAD
    ...
    for (const m of manzanasMarcadas) {
      let list = marcadasByTerritorio.get(m.territorioNumero);
      if (!list) {
        list = [];
        marcadasByTerritorio.set(m.territorioNumero, list);
      }
      list.push(m);
    }
...
<<<<<<< HEAD
  ...
  private countByTerritorio(manzanaIndex: Array<{ territorioNumero: number }>, num: number): number {
    let count = 0;
    for (const m of manzanaIndex) {
      if (m.territorioNumero === num) count++;
    }
    return count;
  }
